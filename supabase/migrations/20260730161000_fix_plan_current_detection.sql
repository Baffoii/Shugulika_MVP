-- Fix activate_employer_package: treat date-lapsed trial/active rows as expired
-- before upgrade checks, so the UI and RPC agree on "current plan".
...
begin
  ...
  -- Lapse any status=trial/active rows that are past their end date (keeps UI/RPC in sync).
