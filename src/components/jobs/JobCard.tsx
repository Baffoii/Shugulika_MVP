  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-ink-muted">
        ...
        <span className="inline-flex items-center gap-1">
          <Briefcase className="h-4 w-4 text-ink-subtle" aria-hidden /> {titleCase(job.employment_type)} · {titleCase(job.work_arrangement)}
        </span>
      </div>
      {job.description ? <p className="mt-3 line-clamp-2 text-sm text-ink-muted">{job.description}</p> : null}
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-surface-border pt-3 text-xs text-ink-subtle">
        ...
        <span className="inline-flex items-center gap-1">
          ...
          {closing ? (
            <span className={closingSoon ? "font-medium text-amber-700" : ""}>Closes {formatDate(closing)} · {relativeDays(closing)}</span>
          ) : (
            "Open"
          )}
        </span>
