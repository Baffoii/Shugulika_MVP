...
export default async function CandidateSettingsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Settings"
        description="Control your discoverability and account preferences."
      />
      <div className="grid gap-4">
        ...
        <Card>
          ...
          <CardBody className="space-y-2 text-sm">
            <p>
              <span className="text-ink-subtle">Sign-in email:</span>{" "}
              <span className="font-medium text-ink">{session.email}</span>
            </p>
            {candidate.contact_email ? (
              <p>
                <span className="text-ink-subtle">Contact email:</span>{" "}
                <span className="font-medium text-ink">{candidate.contact_email}</span>
              </p>
            ) : null}
        ...
        <Alert tone="info" title="Your data">
          You can build one reusable profile and control what is shared. A full data-export and
          account-deletion flow is planned; contact support to request either in the MVP.
        </Alert>
      </div>
    </div>
  );
