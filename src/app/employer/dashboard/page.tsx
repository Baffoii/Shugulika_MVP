      ...
      <div className="mt-6">
        <Card>
          <CardHeader>
            ...
            <Link href="/employer/job-orders" className="text-sm text-brand-700 hover:underline">Manage</Link>
          </CardHeader>
          {jobs.length === 0 ? (
            <div className="p-5"><EmptyState title="No job orders yet" description="Submit a job order to start receiving candidates." /></div>
          ) : (
            <DataTable className="border-0 shadow-none">
              <THead>
                <TR><TH>Role</TH><TH>Route</TH><TH>Status</TH><TH>Vacancies</TH><TH>Created</TH></TR>
              </THead>
              <tbody>
                {jobs.slice(0, 8).map((j) => (
                  <TR key={j.id}>
                    <TD><span className="font-medium text-ink">{j.title}</span></TD>
