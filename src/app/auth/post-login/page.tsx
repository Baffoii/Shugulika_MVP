...
export default async function PostLoginPage({ searchParams }: { searchParams: { redirectTo?: string } }) {
  const session = await getSessionContext();
  if (!session) redirect("/auth/sign-in");
  if (searchParams.redirectTo && searchParams.redirectTo.startsWith("/")) redirect(searchParams.redirectTo);
