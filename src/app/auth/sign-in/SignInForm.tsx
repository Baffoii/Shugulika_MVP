...
export function SignInForm({ redirectTo }: { redirectTo: string | null }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    router.push(redirectTo ? `/auth/post-login?redirectTo=${encodeURIComponent(redirectTo)}` : "/auth/post-login");
    router.refresh();
  }
  ...
  return (
    <form onSubmit={onSubmit} className="mt-5 space-y-4" noValidate>
      {formError ? (
        <p className="rounded-lg border border-red-100 bg-red-50 px-3 py-2 text-sm text-red-700" role="alert">{formError}</p>
      ) : null}
