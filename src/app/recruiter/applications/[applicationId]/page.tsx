...
export default async function ApplicationWorkspace({ params }: { params: { applicationId: string } }) {
  ...
  return (
    <div>
      <Link href="/recruiter/pipeline" className="text-sm text-brand-700 hover:underline">← Back to pipeline</Link>
      <PageHeader
        title={name}
        description={job ? `${job.title} · ${application.recruitment_path === "A" ? "Direct employer" : "Shugulika-managed"}` : undefined}
      ...
      <div className="grid gap-4 lg:grid-cols-3">
          ...
          <Card>
            ...
            <CardBody>
              <div className="flex items-start gap-3">
                <span className="flex h-11 w-11 items-center justify-center rounded-full bg-brand-600 text-sm font-semibold text-white">{initials(name)}</span>
                <div>
                  ...
                  <p className="mt-1 inline-flex items-center gap-1 text-xs text-ink-subtle">
                    <MapPin className="h-3.5 w-3.5" aria-hidden /> {[candidate?.city, candidate?.country_code].filter(Boolean).join(", ") || "—"}
                  </p>
                </div>
              </div>
              {candidate?.summary ? <p className="mt-3 text-sm text-ink-muted">{candidate.summary}</p> : null}
            </CardBody>
          </Card>
                  ...
                  {documents.map((d) => (
                    <li key={d.id} className="flex items-center justify-between gap-3 py-2">
                      <span className="flex items-center gap-2 text-sm text-ink">
                        <FileText className="h-4 w-4 text-ink-subtle" aria-hidden /> {d.title ?? d.object_path.split("/").pop()}
                        {d.is_primary ? <Badge tone="success">Primary CV</Badge> : null}
                      </span>
                      <ViewCvButton bucketId={d.bucket_id} objectPath={d.object_path} label="Open (view-only)" />
                    </li>
                  ))}
                </ul>
              )}
              <p className="mt-2 text-xs text-ink-subtle">Documents open via a short-lived, logged link. Watermarked previews are an integration-pending feature.</p>
            </CardBody>
          </Card>
                  ...
                  {history.map((h) => (
                    <li key={h.id} className="relative">
                      ...
                      <p className="text-sm text-ink">
                        {h.from_stage ? `${titleCase(h.from_stage)} → ` : ""}<span className="font-medium">{titleCase(h.to_stage)}</span>
                      </p>
                      <p className="text-xs text-ink-subtle">
                        {formatDateTime(h.created_at)} · {titleCase(h.actor_role ?? h.source)}{h.reason ? ` · ${h.reason}` : ""}
                      </p>
        ...
        <div className="space-y-4">
          ...
          <Card>
            <CardHeader><CardTitle>Employer submission</CardTitle></CardHeader>
            <CardBody className="space-y-3">
              {submissions.length === 0 ? (
                <EmptyState title="Not submitted" description="Prepare a masked, consent-gated submission for the employer." />
              ) : (
                <ul className="space-y-2">
                  {submissions.map((s) => (
                    <li key={s.id} className="flex items-center justify-between rounded-lg border border-surface-border px-3 py-2 text-sm">
