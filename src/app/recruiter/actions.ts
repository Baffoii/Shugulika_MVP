...
async function writeAudit(action: string, entityId: string, orgId: string | null, before: unknown, after: unknown) {
  const supabase = createClient();
  const actorId = await actor();
  await supabase.from("audit_logs").insert({
    actor_id: actorId, action, entity_type: "application", entity_id: entityId, org_context_id: orgId,
    before_value: before as never, after_value: after as never,
  });
}
...
export async function advanceStageAction(formData: FormData): Promise<ActionResult> {
  ...
  if (toStage === "rejected") {
    ...
    const reasonLabel = REJECTION_REASONS.find((r) => r.key === rejectionReason)?.label ?? rejectionReason;
    await supabase.from("applications").update({ current_stage: "applied_sourced", is_on_hold: false }).eq("id", applicationId);
    await supabase.from("application_stage_history").insert({
      application_id: applicationId, from_stage: app.current_stage, to_stage: "rejected",
      actor_id: await actor(), actor_role: "recruiter", reason: reasonLabel, note: note || null, source: "recruiter",
    });
    await writeAudit("application.rejected", applicationId, app.owning_org_id, { stage: app.current_stage }, { stage: "rejected", reason: reasonLabel });
  ...
  if (!target || target.stageClass !== "candidate") return { ok: false, error: "Invalid target stage." };
  ...
  if (toStage === "client_submission") {
    ...
    if (!count || count === 0) {
      return { ok: false, error: "Create an employer submission (with candidate consent) before moving to Client Submission." };
    }
  }

  await supabase.from("applications").update({ current_stage: toStage }).eq("id", applicationId);
  await supabase.from("application_stage_history").insert({
    application_id: applicationId, from_stage: app.current_stage, to_stage: toStage,
    actor_id: await actor(), actor_role: "recruiter", note: note || null, source: "recruiter",
  });
  await writeAudit("application.stage_changed", applicationId, app.owning_org_id, { stage: app.current_stage }, { stage: toStage });
...
async function notifyCandidate(app: ApplicationRow, title: string, body: string) {
  const supabase = createClient();
  const { data: cand } = await supabase.from("candidate_profiles").select("user_id").eq("id", app.candidate_id).maybeSingle();
  const userId = (cand as { user_id: string } | null)?.user_id;
  if (userId) {
    await supabase.from("notifications").insert({
      user_id: userId, category: "application_status", title, body, subject_type: "application", subject_id: app.id,
    });
  }
}
...
export async function addNoteAction(formData: FormData): Promise<ActionResult> {
  ...
  const { error } = await supabase.from("recruiter_notes").insert({
    subject_type: "application", subject_id: applicationId, owning_org_id: app.owning_org_id,
    author_id: actorId, body, visibility,
  });
...
export async function createSubmissionAction(formData: FormData): Promise<ActionResult> {
  ...
  const { data: jo } = await supabase.from("job_orders").select("employer_org_id").eq("id", app.job_order_id).maybeSingle();
  ...
  const { data: cand } = await supabase.from("candidate_profiles").select("*").eq("id", app.candidate_id).maybeSingle();
  ...
  const { data: sub, error } = await supabase
    .from("employer_submissions")
    .insert({
      application_id: applicationId, candidate_id: app.candidate_id, job_order_id: app.job_order_id,
      employer_org_id: employerOrgId, submitting_org_id: app.owning_org_id, submitting_recruiter_id: await actor(),
      consent_id: (consent as { id: string } | null)?.id ?? null,
      status: hasConsent ? "submitted" : "consent_pending", is_masked: true, summary: summary || null,
      disclosed_profile: disclosed as never, disclosed_fields: ["headline", "location", "summary", "availability"],
      cv_document_id: app.cv_document_id, submitted_at: hasConsent ? new Date().toISOString() : null,
    })
    .select("id")
    .single();
  if (error) return { ok: false, error: error.message };

  await supabase.from("applications").update({ consent_status: hasConsent ? "granted" : "pending" }).eq("id", applicationId);
  await writeAudit("submission.created", (sub as { id: string }).id, app.owning_org_id, null, { employer: employerOrgId, consent: hasConsent });

  // Ask the candidate for employer-specific consent when it's missing.
  if (!hasConsent) {
    await notifyCandidate(app, "Action required: approve client submission", "A recruiter would like to submit your profile to an employer. Please review and approve.");
  }
